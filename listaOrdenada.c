...
void Insertion(tp_lista *head){
	...
	//Verifica se a lista possui elementos para Ordenar
	if(head->first == NULL || head->nItens < 2) {
		system("clear");
		printf("\t___________________~<>~__________________\n");
		...
		for(a = a->next; a != NULL; a = a->next){ //for que percorre a lista
			for(b = a->prev; b != NULL; b = b->prev){ //for que percorre a lista a partir do anterior do first
				if(a->produto.codigo < b->produto.codigo){ //compara se o codigo é maior
					if(head->nItens == 2) { //Apenas dois elementos na lista
						b->prev = a;
						a->next = b;
					...
					else if(b->prev == NULL){ //caso seja o primeiro trocando
						a->next->prev = b;
						b->next = a->next;
...
void Selection(tp_lista *head){
	...
	for(i = head->first; i != head->last; a = i = i->next){ //for que percorre a lista para achar o menor
		b = i;
